...
namespace InuCommon
{
    ...
    public:
        ...
        CInuDevice(const std::string& iSensorID, const std::string& iSensorModel, InuCommon::CTiffFile::EHWType iHwType);

        virtual ~CInuDevice() { }

        /// \brief    Initialize the object and prepare it for working, must be call after construction
        EInuSensorsDBErrors Init();

        /// \brief    Load calibration data
        /// \param[in] iRequiredTemperatures    List of all temperatures that should have calibration data.
        ///                                     All missing calibration should be calculated by the load function
        /// \param[in] iDirectoryName    Enable to load calibration files from different directory.
        /// \param[in] iSaturationMin   Minimum Saturation value for interpolation/extrapolation
        /// \param[in] iSaturationMax   Maximum Saturation value for interpolation/extrapolation
        /// \param[in] iDirectoryName   Calibration directory name
        /// \param[in] iTemperatureCompenstation   Whether to invoke temperature compensation or not
        EInuSensorsDBErrors LoadCalibration(const std::vector<int>& iRequiredTemperatures,
            int iSaturationMin,
            int iSaturationMax,
            const std::string& iDirectoryName,
        ...
        const CInuModel& SensorModel() const 
        { 
            return mSensorModel;
        }
        ...
        void SetPartNumber(const std::string& iPartNumber) { mPartNumber = iPartNumber; }

        const std::string& SensorRevision() const { return mSensorRevision; }
        void SetSensorRevision(const std::string& iSensorRevision) { mSensorRevision = iSensorRevision; }
        ...
        void SetRevisionID(unsigned int iInput);

        const std::vector<int>& GetMasterSensorId() const { return mMasterSensorId; }
        void SetMasterSensorId(const std::vector<int>& iID) { mMasterSensorId = iID; }
        ...
        void SetCalibrationDataSize(uint32_t iInput) { mCalibrationDataSize = iInput; }

        uint32_t  GetCalibrationDataFormat() const { return mCalibrationDataFormat; }
        void SetCalibrationDataFormat(uint32_t iInput) { mCalibrationDataFormat = iInput; }

        uint64_t  GetLastBurnTime() const { return mLastBurnTime; }
        void SetLastBurnTime(uint64_t iInput) { mLastBurnTime = iInput; }

        const std::string& CalibrationVersion() const { return mCalibrationVersion; }
        ...
        void SetBootfixVersion(const std::string& iInput) { mBootfixVersion = iInput; }

        bool GetDualSensorsMaster() const { return mDualSensorsMaster; }
        void SetDualSensorsMaster(bool iIsMaster) { mDualSensorsMaster = iIsMaster; }

        uint32_t  GetAudioFreq() const { return mAudioFreq; }
        void SetAudioFreq(uint32_t iInput) { mAudioFreq = iInput; }

        uint32_t  GetCpuFreq() const { return mCpuFreq; }
        void SetCpuFreq(uint32_t iInput) { mCpuFreq = iInput; }

        uint32_t  GetDdrFreq() const { return mDdrFreq; }
        void SetDdrFreq(uint32_t iInput) { mDdrFreq = iInput; }

        uint32_t  GetDspFreq() const { return mDspFreq; }
        void SetDspFreq(uint32_t iInput) { mDspFreq = iInput; }

        uint32_t  GetSysFreq() const { return mSysFreq; }
        void SetSysFreq(uint32_t iInput) { mSysFreq = iInput; }

        uint32_t  GetUsb2Freq() const { return mUsb2Freq; }
        void SetUsb2Freq(uint32_t iInput) { mUsb2Freq = iInput; }

        uint32_t  GetUsb3Freq() const { return mUsb3Freq; }
        void SetUsb3Freq(uint32_t iInput) { mUsb3Freq = iInput; }

        /// \return bool    true if there is a match between SWRevision and local calibration folder
        ...
        /// \return std::string    Local path of current sensor's data or empty string if it doesn't exist
        static std::string LocalPath(const std::string& iSensorID);

        /// \brief    brief Check if LUTs exist for the input sensor. 
        /// \param[in] iSensorNumber    Sensor Number (as is defined in the calibration data)
        /// \return bool    True if LUTs exist
        bool LutExist(uint32_t iSensorNumber, CTiffFile::ESensorResolution iResolution = CTiffFile::eUnknownResolution) const;

        bool DepthRegistrationCalibrationExits(uint32_t isensorId, CTiffFile::ESensorResolution iResolution) const;
    ...
    private:
        ...
        /// \brief  If true then either it's not a Dual Sensors configuration or this is the Master InuSesnor of Dual Sensors configuration
    ...
    {
        return (mCalibrationDataAll != nullptr) && mCalibrationDataAll->TemperatureCalibration();
    }

    inline unsigned int  CInuDevice::GetRevisionID() const
    {
        return mRevisionID; 
    }
    
    inline void CInuDevice::SetRevisionID(unsigned int iInput)
    { 
        mRevisionID = iInput; 
    }
}
