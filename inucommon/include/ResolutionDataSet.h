...
namespace InuCommon
{
    ...
    public:
        ...
        EInuSensorsDBErrors Load(const boost::filesystem::path& iResolutionSetDir, InuCommon::CTiffFile::EHWType iHwType, bool iLoadOdpReg, const CCalibrationData* originalOptical);
        ...
        const std::vector<uint8_t>& GetLUT(CLutsDataSet::ELutType iType, uint32_t iSensorName, uint32_t iLutIndex = 0) const {
            return mLutsDataSet.GetLUT(iType, iSensorName, iLutIndex);
        }
        ...
        std::shared_ptr<CCalibrationData> GetOpticalDataAndUpdateSensorsID(
            const std::pair<int, int>& iStereoChannels,
            const std::pair<int, int>& iFeChannels,
            const int iRgbChannel);

        std::shared_ptr<const CCalibrationData> GetCalibrationData() const { return mCalibrationData; }
...
#endif
        std::shared_ptr<CCalibrationData> mCalibrationData;
