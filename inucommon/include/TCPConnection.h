...
#endif
#endif
// This definition comes to eleminate many warnings from boost of [-Wunused-variable]
...
namespace InuCommon 
{
    ...
    public:
        ...
        void Write(const std::string& ip, std::vector<unsigned char>& writeBuffer) const;
        void Write(unsigned long ipV4, std::vector<unsigned char>& writeBuffer) const;
    ...
    public:
        CTcpClientConnection(boost::asio::ip::tcp::socket& socket, boost::asio::io_service& io_service) :
            m_io_service(io_service), m_socket(socket) 
        {}

        ~CTcpClientConnection() {};

        int Write(const std::string& ip, std::vector<unsigned char>& writeBuffer) const;

        int Write(unsigned long ipV4, std::vector<unsigned char>& writeBuffer) const;
    ...
    public:
        ...
        EErrorCode Read(std::vector<unsigned char>& readBuffer, unsigned long& processID, unsigned long timeout);
        ...
        // After it accepted a connection form client application
        bool IsListening() const { return mIsListening; }

        struct TcpLockPair
        {
            std::shared_ptr<CTcpClientConnection> tcpConnector;
            std::shared_ptr<boost::shared_mutex> lock; //Lock each connection for atomic sends
        };
