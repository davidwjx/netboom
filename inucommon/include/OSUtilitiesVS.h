...
namespace InuCommon {
    ...
    {
        std::wstring result;
        int size = int(iFilename.size());
        std::unique_ptr<wchar_t> tmp(new wchar_t[size+1]);
        if (MultiByteToWideChar(CP_UTF8, 0, iFilename.c_str(), size, tmp.get(), size) == size)
        {
            tmp.get()[size] = L'\0';
            result = tmp.get();
        }
    ...
    inline std::string COsUtilities::ConvertEncodingBack(const std::wstring& iFilename)
    {
        std::string result;
        int size = int(iFilename.size());
        std::unique_ptr<char> tmp(new char[size+1]);
        if (WideCharToMultiByte(CP_UTF8, 0, iFilename.c_str(), size, tmp.get(), size,NULL, NULL) == size)
        {
            tmp.get()[size] = '\0';
            result = tmp.get();
        }
