...
//
// Inuitive fix:
// Use timeout with spinlock (do_send line 756)
//
//////////////////////////////////////////////////////////////////////////////
    ...
    namespace interprocess {
            ...
            unsigned int priority, const boost::posix_time::ptime &abs_time)
        {
...
#endif
            //---------------------------------------------
            scoped_lock<interprocess_mutex> lock(p_hdr->m_mutex, abs_time); //Inuitive add timed lock (we only use timed_send)
            if (!lock.owns())
            {
                return false;
            }
            ...
            if (notify_blocked_receivers) {
                p_hdr->m_cond_recv.notify_one(abs_time);  //Inuitive add timed notification
            }
#else
            p_hdr->m_cond_recv.notify_one(abs_time);  //Inuitive add timed notification
#endif
                ...
                const boost::posix_time::ptime &abs_time)
        {
            ...
            if (notify_blocked_senders) {
                p_hdr->m_cond_send.notify_one(abs_time); //Inuitive add timed lock (we only use timed_send)
            }
#else
            p_hdr->m_cond_send.notify_one(abs_time); //Inuitive add timed lock (we only use timed_send)
#endif
