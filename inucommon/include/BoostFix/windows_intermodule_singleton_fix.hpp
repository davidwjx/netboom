...
//
// Inuitive fix:
// Fixed compatibility issue with previous boost versions (version < 1.54)
// Interprocess communication uses this intermodule singleton for whatever. (related bug report: https://svn.boost.org/trac/boost/ticket/9767)
// 
// Changed the names to include "InuDev", this promises our own map, so we don't clash with different boost versions in the same process
//
//
//////////////////////////////////////////////////////////////////////////////
...
class windows_semaphore_based_map
{
   ...
   windows_semaphore_based_map()
   {
      ...
      scoped_lock<winapi_mutex_wrapper> lck(m_mtx_lock);
      {
         ...
         name = "bipc_gmap_sem_map_InuDev_";
         name += pid_creation_time;
