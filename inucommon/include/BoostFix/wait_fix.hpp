...
//
// Inuitive fix: 
// Added NUM_OF_YIELDS - more yields before sleep, better latency in long waits
//
...
#ifndef __ANDROID__
#define NUM_OF_YIELDS 50
#else
#define NUM_OF_YIELDS 400
#endif
...
class spin_wait
{
   ...
   bool yield_or_sleep()
   {
      if(!m_ul_yield_only_counts){  //If yield-only limit was reached then yield one in every NUM_OF_YIELDS tries
         return (m_k % NUM_OF_YIELDS) != 0;
      }
