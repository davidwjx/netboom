...
//
// Inuitive fix:
// Add timeout to spinlock (notify_one, notify_all)
//
//////////////////////////////////////////////////////////////////////////////
...
namespace boost {
    namespace interprocess {
        namespace ipcdetail {
            ...
            public:
                ...
                bool notify_one(const boost::posix_time::ptime &abs_time);
                bool notify_all(const boost::posix_time::ptime &abs_time);
   ...
   bool notify(boost::uint32_t command, const boost::posix_time::ptime &abs_time); //Inuitive add timed notification
            };
            ...
            inline spin_condition::~spin_condition()
            {
                ...
                this->notify_all(boost::posix_time::pos_infin); //Inuitive add timed notification
            }

inline bool spin_condition::notify_one(const boost::posix_time::ptime &abs_time) //Inuitive add timed notification
{
   return this->notify(NOTIFY_ONE, abs_time);
            }

inline bool spin_condition::notify_all(const boost::posix_time::ptime &abs_time) //Inuitive add timed notification
{
   return this->notify(NOTIFY_ALL, abs_time);
            }

inline bool spin_condition::notify(boost::uint32_t command, const boost::posix_time::ptime &abs_time) //Inuitive add timed notification
            {
   ...
   //m_enter_mut.lock();
   //Fix the situation where one process dies while having m_enter_mut locked 
   //by providing a timeout on the lock call 
    if(!m_enter_mut.timed_lock(abs_time)) //Inuitive add timed notification
    { 
        return false; 
    } 
   ...
   return true;
                ...
                InterprocessMutex &mut)
            {
                ...
                bool lockAcquired = mut.timed_lock(abs_time); //Inuitive add timed lock (we only use timed_send)
                return !timed_out && lockAcquired;
            }
