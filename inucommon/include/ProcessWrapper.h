...
namespace InuCommon
{
    ...
    public:
        ...
        EResultCodes RunCommand(const std::string & iCmd, bool iKeepOutput = false, bool iAlwaysUsePopen = false);

        /// \brief Run an external executable, check that it exists
        EResultCodes RunProcess(const std::string & iProcess, const std::string & iArgs, bool iKeepOutput = false);
        
        /// \brief Run an external script using an interpreter, check that both exists
        EResultCodes RunScript(const std::string & iInterpeter, const std::string & iScript, const std::string & iArgs, bool iKeepOutput = false);
...
#ifdef _WIN32
        EResultCodes RunProcessWindows(const std::string & iCmd, bool iKeepOutput = false);
#endif
        EResultCodes RunProcess_popen(const std::string & iCmd, bool iKeepOutput = false);
