...
namespace InuCommon { 
    ...
    inline bool COsUtilities::WaitForThreadTermination(pthread_t& hThread,unsigned int timeout) //out in miliseconds
    {
        DWORD exitCode;
    ...
    inline bool COsUtilities::IsThreadAlive(pthread_t& iThread)
    {
        ...
        // If we wait for more than 0, then the thread will join and the memory will be cleaned
        // !! The timeout must be at least 1 ms, otherwise it doesn't do join on Linux X64 !!
    ...
    {
       // if we want to sleep for more than 1 second - we need to call sleep because
        // usleep is limited to up to 1,000,000 microseconds.
    ...
    inline void* COsUtilities::AllocteBuffer(unsigned int iSize, unsigned int iAlignment)
    {
        char* buffer;
        if (iAlignment == 1)
        {
