...
namespace InuCommon 
{
    ...
    /// \brief Compile time trim basename(filename)
    using cstr = const char * const;
    static constexpr cstr past_last_slash(cstr str, cstr last_slash)
    {
        return
            *str == '\0' ? last_slash :
            *str == InuCommon::COsUtilities::PATH_SEPARATOR ? InuCommon::past_last_slash(str + 1, str + 1) :
            InuCommon::past_last_slash(str + 1, last_slash);
    }

    static constexpr cstr past_last_slash(cstr str)
    {
        return InuCommon::past_last_slash(str, str);
    }

#define STRINGIZE_DETAIL(x) #x
#define STRINGIZE(x) STRINGIZE_DETAIL(x)

#define CONCAT_FILE_LINE(message) message << " [" << InuCommon::past_last_slash(__FILE__) << (":" STRINGIZE(__LINE__) "]")

#define LOGGER_WRITE(message, level, appName) \
	if (InuCommon::CLogger::CheckSeverity(level, appName)) \
	{ \
        std::ostringstream strStream; \
        strStream << "--- Inu Debug at: " << CONCAT_FILE_LINE(message); \
		InuCommon::CLogger::Write(strStream.str(), level, appName); \
	}

#define LOGGER_DEBUG(message, appName) LOGGER_WRITE(message, InuCommon::CLogger::eDebug, appName) 
#define LOGGER_INFO(message, appName) LOGGER_WRITE(message, InuCommon::CLogger::eInfo, appName) 
#define LOGGER_WARNING(message, appName) LOGGER_WRITE(message, InuCommon::CLogger::eWarning, appName) 
#define LOGGER_ERROR(message, appName) LOGGER_WRITE(message, InuCommon::CLogger::eError, appName) 
#define LOGGER_STATUS(message, appName) LOGGER_WRITE(message, InuCommon::CLogger::eStatus, appName) 
    ...
    public:
        ...
        ///             If iForce is false then only the first call of this function takes effect.
        static void SetLogType(int iLogType = eAuto, bool iForce = false);
        ...
        static std::string GetFileLoggerPath();
    ...
    private:
        ...
        std::list<std::pair<CLogger::ESeverity, std::string> > mMessagesQueue; //log message queue
