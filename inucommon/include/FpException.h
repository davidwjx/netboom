...
#else
    // This code has no affect in Linux... FP_DIVISION is recognized, by never an exception is generated.
...
#else
    FPExceptionEnabler(fexcept_t enableBits = FE_DIVBYZERO | FE_OVERFLOW | FE_INVALID)
#endif
    {
        // Retrieve the current state of the exception flags. This
        // must be done before changing them. _MCW_EM is a bit
