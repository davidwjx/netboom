...
{
    ////////////////////////////////////////////////////////////////////////
    /// \brief    Enhanced Optical Data which is used for Disparity2Depth calculation
    ///
    /// Responsibilities:
    ///      1. Disparity image data
    ///      2. Knows how to calculate Optical Factor and Disparity Offset
    ////////////////////////////////////////////////////////////////////////
    class CDepthOpticalData : public CCalibrationData
    {
    public:

        /// \brief def Constructor.
        /// \param[in] iOpticalData    Optical Data object.
        /// \param[in] iStereoSensors    Stereo sensors ids: first is depth match (right)
        CDepthOpticalData()
        ...
        /// \param[in] iOpticalData    Optical Data object.
        /// \param[in] iStereoSensors    Stereo sensors ids: first is depth match (right)
        /// \param[in] iBaseline   Distance between stereo sensors
        /// \param[in,out] iDownScaleFactor
        /// \param[in,out] iHorizPadding
        /// \param[in,out] iVerticalPadding
        CDepthOpticalData(
                const CCalibrationData& iOpticalData,
        ...
        {
            mOpticalFactor = float(2 *
                Sensors[iStereoSensors.second].VirtualCamera.Intrinsic.FocalLength[0] * 
                iBaseline / mDownScaleFactor);        // factor 2 is because disparity is on half pixel units
            mDisparityOffset = float(2 *
                (Sensors[iStereoSensors.first].VirtualCamera.Intrinsic.OpticalCenter[0]  - Sensors[iStereoSensors.second].VirtualCamera.Intrinsic.OpticalCenter[0]) /
                mDownScaleFactor - 
                2 * mHorizPadding);
        }

        std::array<double, 2> GetFocalLengthRealRight() const
        {
            return {
                float(Sensors.at(mStereoSensors.first).RealCamera.Intrinsic.FocalLength[0]),
                float(Sensors.at(mStereoSensors.first).RealCamera.Intrinsic.FocalLength[1]) };
        }
