...
{ 
    ...
    struct COMMONUTILITIES_API CRecordingMetaData : public IPersistentData
    {
        ...
        CCalibrationData mOpticalData;
        std::string mOpticalDataAsString;

        std::string mRegisters;

        virtual bool  Load(const std::shared_ptr<InuCommon::IStorage>& storage) override;

        virtual bool  Save(const std::shared_ptr<InuCommon::IStorage>& storage) override; 
