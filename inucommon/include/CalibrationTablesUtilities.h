...
{ 
    ...
    public:
        ...
        /// \param[out] oCompressedFileName Output full path of compressed file
        /// \return     ECompressionError 
        static ECompressionError Compress(const std::string& iCalibrationData, 
            std::vector<unsigned char>& oBuffer,
            std::string& oCompressedFileName);

        /// \brief    Compress input camera calibration folder into output unsigned char buffer.
        ///
        /// \param[in]  iBuffer             Buffer to decompress
        /// \param[in]  iBufferSize         Buffer size
        /// \param[in]  iSensorID           Sensor ID
        /// \param[in]  iLastBurnTime       Last time that the sensor was flashed
        /// \param[in]  iDestinationPath    Where to save the calibration folder (default is in the installation config folder)
        /// \param[out] oCreateDirName      Created file name.
        /// \return     ECompressionError 
        static ECompressionError  Decompress(std::string& oCreateDirName,
            const char* iBuffer,
        ...
        /// \brief    Same description as previous function, but doesn't return  the created calibration dir name.
