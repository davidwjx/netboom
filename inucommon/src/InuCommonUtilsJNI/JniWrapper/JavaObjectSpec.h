...
inline void CJavaObject::constructObject(JNIEnv * env, std::string typeName, jobject iClassLoader , std::string constructorSignature, Args... args)
{
	clearClass = true;
	...
	else
	{
        ...
        CJavaMember findClassMethod = reflector["findClass"];
        findClassMethod[std::string("(Ljava/lang/String;)Ljava/lang/Class;")](&reflectorClassObject, javaString);
		mClassTypeReflector = (jclass)reflectorClassObject;
 		mEnvironment->DeleteLocalRef(javaString);
	}
	jmethodID classConstructor = env->GetMethodID(mClassTypeReflector, "<init>", constructorSignature.c_str() );
...
template <>
inline CJavaMember& CJavaMember::operator = <char>(char value) {
...
template <>
inline char CJavaMember::cast<char>() const  {
    jfieldID fid = mEnvironment->GetFieldID(mClassTypeReflector, mName.c_str(), "C");
...
template <>
inline jstring CJavaMember::cast<jstring>() const {
    jfieldID fid = mEnvironment->GetFieldID(mClassTypeReflector, mName.c_str(), "Ljava/lang/String;");
    return (jstring)mEnvironment->GetObjectField(mObject,fid);
}
