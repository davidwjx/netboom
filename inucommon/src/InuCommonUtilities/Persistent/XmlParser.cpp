// Markup.cpp: implementation of the CXmlParser class.
//
//
...
void x_AddResult( MCD_STR& strResult, MCD_CSTR pszID, MCD_CSTR pszVal = NULL, int nResultCode = 0, int n = -1, int n2 = -1 )
{
    ...
    {
        // Use a temporary CXmlParser object but keep strResult in a string to minimize memory footprint
        CXmlParser mResult( strResult );
        if ( nResultCode & MRC_MODIFY )
            mResult.FindElem( pszID );
        else
            mResult.AddElem( pszID, MCD_T(""), CXmlParser::MNF_WITHNOLINES );
        if ( pszVal.pcsz )
        {
...
#else
            nResult = iconv( cd, (const char**) &pFromChar, &nFromLenRemaining, &pToChar, &nToCountRemaining );
#endif			
...
int TextEncoding::PerformConversion( void* pTo, MCD_CSTR pszToEncoding/*=NULL*/ )
{
    ...
    if ( nFromCP == MCD_ACP )
    {
        ...
        while ( nALenRemaining )
        {
            ...
            if ( nToCP == MCD_UTF8 )
                CXmlParser::EncodeCharUTF8( (int)wcChar, pU, nToLen );
            else if ( nToCP == MCD_UTF16 )
                CXmlParser::EncodeCharUTF16( (int)wcChar, (unsigned short*)pU, nToLen );
            else // UTF32
            {
                if ( pU )
    ...
    else if ( nToCP == MCD_ACP )
    {
        ...
        {
            if ( nFromCP == MCD_UTF8 )
                nUChar = CXmlParser::DecodeCharUTF8( pU.p8, pUEnd );
            else if ( nFromCP == MCD_UTF16 )
                nUChar = CXmlParser::DecodeCharUTF16( pU.p16, (const unsigned short*)pUEnd );
            else // UTF32
                nUChar = *(pU.p32)++;
            if ( nUChar == -1 )
                nCharLen = -2;
    ...
    if ( nFromCP == MCD_UTF32 )
    {
        ...
        if ( nToCP == MCD_UTF8 )
        {
            char* p8 = (char*)pTo;
            while ( p32 != p32End )
                CXmlParser::EncodeCharUTF8( *p32++, p8, nToLen );
        }
        else if ( nToCP == MCD_UTF16 )
        {
            unsigned short* p16 = (unsigned short*)pTo;
            while ( p32 != p32End )
                CXmlParser::EncodeCharUTF16( (int)*p32++, p16, nToLen );
        }
        else // to ANSI
        {
    ...
    else if ( nFromCP == MCD_UTF16 )
    {
        ...
        if ( nToCP == MCD_UTF32 )
        {
            ...
            while ( p16 != p16End )
            {
                nUChar = CXmlParser::DecodeCharUTF16( p16, p16End );
                if ( nUChar == -1 )
                    nUChar = '?';
                if ( p32 )
        ...
        else if ( nToCP == MCD_UTF8 )
        {
            ...
            while ( p16 != p16End )
            {
                ...
                if ( nUChar == -1 )
                    nUChar = '?';
                CXmlParser::EncodeCharUTF8( nUChar, p8, nToLen );
            }
        }
        else // to ANSI
        {
    ...
    else if ( nToCP == MCD_UTF16  ) // to UTF-16 from UTF-8/ANSI
    {
        ...
        if ( nFromCP == MCD_UTF8 )
        {
            ...
            while ( p8 != p8End )
            {
                nUChar = CXmlParser::DecodeCharUTF8( p8, p8End );
                if ( nUChar == -1 )
                    nUChar = '?';
                if ( p16 )
...
bool TextEncoding::FindRaggedEnd( int& nTruncBeforeBytes )
{
    ...
    if ( nCP == MCD_UTF16 )
    {
        ...
        if ( CXmlParser::DecodeCharUTF16(pUTF16Last,&pUTF16Buffer[m_nFromLen]) == -1 )
            nTruncBeforeBytes = 2;
    }
    else // UTF-8, SBCS DBCS
    {
        if ( nCP == MCD_UTF8 )
        {
            ...
            while ( nLast > 0 && CXmlParser::DecodeCharUTF8(pUTF8Last,pUTF8End) == -1 )
...
bool x_EndianSwapRequired( int nDocFlags )
{
    ...
    if ( cFirstByte ) // LE
    {
        if ( nDocFlags & CXmlParser::MDF_UTF16BEFILE )
            return true;
    }
    else if ( nDocFlags & CXmlParser::MDF_UTF16LEFILE )
        return true;
    return false;
}
...
struct InuCommon::TokenPos
{
    ...
    {
        int nLen = Length();
        if ( m_nTokenFlags & CXmlParser::MDF_IGNORECASE )
...
void ElemStack::PushTagAndCount( TokenPos& token )
{
    ...
    if ( iTop != iPar )
    {
        ...
        {
            nSlot = CalcSlot( pTagName, token.Length(), (token.m_nTokenFlags & CXmlParser::MDF_IGNORECASE)?true:false );
...
// In file mode the file stays open between CXmlParser calls and is stored in m_pFilePos
//
struct InuCommon::FilePos
{
    FilePos()
    {
...
{
    { "\xef\xbb\xbf", 3, MCD_T("UTF-8"), CXmlParser::MDF_UTF8PREAMBLE },
    { "\xff\xfe", 2, MCD_T("UTF-16LE"), CXmlParser::MDF_UTF16LEFILE },
    { "\xfe\xff", 2, MCD_T("UTF-16BE"), CXmlParser::MDF_UTF16BEFILE },
    { NULL,0,NULL,0 }
};
...
bool FilePos::FileAtTop()
{
    ...
    if ( ((m_nDocFlags & CXmlParser::MDF_READFILE) && m_nFileByteOffset < (MCD_INTFILEOFFSET)m_nOpFileByteLen + 5 )
            || ((m_nDocFlags & CXmlParser::MDF_WRITEFILE) && m_nFileByteOffset < 5) )
        return true;
    return false;
}
...
bool FilePos::FileOpen( MCD_CSTR_FILENAME szFileName )
{
    ...
    if ( m_nDocFlags & CXmlParser::MDF_APPENDFILE )
        pMode = MCD_T_FILENAME("ab");
    else if ( m_nDocFlags & CXmlParser::MDF_WRITEFILE )
    ...
    if ( m_nDocFlags & CXmlParser::MDF_READFILE )
    {
        // Get file length
        ...
        if ( nReadTop )
        {
            ...
            if ( bSuccess )
            {
                ...
                int nBomCheck = 0;
                m_nDocFlags &= ~( CXmlParser::MDF_UTF16LEFILE | CXmlParser::MDF_UTF8PREAMBLE );
...
#endif // not WCHAR
                    m_strEncoding = CXmlParser::GetDeclaredEncoding( strDeclCheck );
                }
...
bool FilePos::FileReadText( MCD_STR& strDoc )
{
    ...
    if ( m_nDocFlags & (CXmlParser::MDF_UTF16LEFILE | CXmlParser::MDF_UTF16BEFILE) )
    {
    ...
    else // single or multibyte file (i.e. not UTF-16)
    {
        ...
        if ( MCD_STRISEMPTY(m_strEncoding) )
        {
            int nNonASCII;
            bool bErrorAtEnd;
            if ( CXmlParser::DetectUTF8(pBuffer,m_nOpFileByteLen,&nNonASCII,&bErrorAtEnd) || (bCheckRaggedEnd && bErrorAtEnd) )
            {
                m_strEncoding = MCD_T("UTF-8");
        ...
        else // load directly into string
        {
            ...
            if ( bAssumeUnknownIsNative )
            {
                ...
                int nNonASCII;
                bool bErrorAtEnd;
                bool bIsUTF8 = CXmlParser::DetectUTF8( pBuffer, m_nOpFileByteLen, &nNonASCII, &bErrorAtEnd ) || (bCheckRaggedEnd && bErrorAtEnd);
...
bool FilePos::FileClose()
{
    if ( m_fp )
    {
        if ( fclose(m_fp) )
            FileErrorAddResult();
        m_fp = NULL;
        m_nDocFlags &= ~(CXmlParser::MDF_WRITEFILE|CXmlParser::MDF_READFILE|CXmlParser::MDF_APPENDFILE);
        return true;
    }
    return false;
}
...
bool FilePos::FileFlush( MCD_STR& strBuffer, int nWriteStrLen/*=-1*/, bool bFflush/*=false*/ )
{
    ...
    if ( nWriteStrLen )
    {
        ...
        {
            m_strEncoding = CXmlParser::GetDeclaredEncoding( strBuffer );
            if ( MCD_STRISEMPTY(m_strEncoding) )
                m_strEncoding = MCD_T("UTF-8");
        }
...
bool PathPos::AttribPredicateMatch( TokenPos& token )
{
    ...
    if ( token.FindAttrib(GetPtr()) )
    {
        ...
        if ( GetChar() == '=' )
        {
            ...
            MCD_STR strPathValue = CXmlParser::UnescapeText( pszVal, GetValOrWordLen() );
            MCD_STR strAttribValue = CXmlParser::UnescapeText( token.GetTokenPtr(), token.Length() );
...
struct InuCommon::SavedPosMapArray
{
    ...
    while ( 1 )
    {
        ...
        if ( ! cD )
        {
            ...
            if ( ! cD )
            {
                ...
                if ( nNodeType != CXmlParser::MNT_WHITESPACE && nNodeType != CXmlParser::MNT_TEXT )
                {
                    MCD_PCSZ pType = MCD_T("tag");
                    if ( (nParseFlags & PD_DOCTYPE) || nNodeType == CXmlParser::MNT_DOCUMENT_TYPE )
                        pType = MCD_T("document_type");
                    else if ( nNodeType == CXmlParser::MNT_ELEMENT )
                        pType = MCD_T("start_tag");
                    else if ( nNodeType == 0 )
                        pType = MCD_T("end_tag");
                    else if ( nNodeType == CXmlParser::MNT_CDATA_SECTION )
                        pType = MCD_T("cdata_section");
                    else if ( nNodeType == CXmlParser::MNT_PROCESSING_INSTRUCTION )
                        pType = MCD_T("processing_instruction");
                    else if ( nNodeType == CXmlParser::MNT_COMMENT )
                        pType = MCD_T("comment");
                    nNodeType = -1;
        ...
        if ( pFindEnd )
        {
            if ( cD == '>' && ! (nParseFlags & (PD_INQUOTE_S|PD_INQUOTE_D)) )
            {
                ...
                {
                    pFindEnd = NULL;
                    if ( nNodeType == CXmlParser::MNT_ELEMENT && cDminus1 == '/' )
                    {
            ...
            else if ( cD == '<' && (nNodeType == CXmlParser::MNT_TEXT || nNodeType == -1) )
            {
                m_nNext = (int)(pD - m_pDocText);
                break;
            }
            ...
            else if ( nNodeType & CXmlParser::MNT_DOCUMENT_TYPE )
            {
        ...
        else if ( nParseFlags )
        {
            if ( nParseFlags & PD_TEXTORWS )
            {
                ...
                {
                    m_nNext = (int)(pD - m_pDocText);
                    nNodeType = CXmlParser::MNT_WHITESPACE;
                    break;
                }
                else if ( ! MCD_PSZCHR(MCD_T(" \t\n\r"),(MCD_CHAR)cD) )
                {
                    nParseFlags ^= PD_TEXTORWS;
                    FINDNODETYPE( MCD_T("<"), CXmlParser::MNT_TEXT )
                }
            }
            else if ( nParseFlags & PD_OPENTAG )
            {
                ...
                if ( cD > 0x60 || ( cD > 0x40 && cD < 0x5b ) || cD == 0x5f || cD == 0x3a )
                    FINDNODETYPENAME( MCD_T(">"), CXmlParser::MNT_ELEMENT, 0 )
                else if ( cD == '/' )
                    FINDNODETYPENAME( MCD_T(">"), 0, 1 )
                else if ( cD == '!' )
                    nParseFlags |= PD_BANG;
                else if ( cD == '?' )
                    FINDNODETYPENAME( MCD_T("?>"), CXmlParser::MNT_PROCESSING_INSTRUCTION, 1 )
                else
            ...
            else if ( nParseFlags & PD_BANG )
            {
                ...
                else if ( nParseFlags & PD_DOCTYPE )
                {
                    if ( MCD_PSZCHR(MCD_T("EAN"),(MCD_CHAR)cD) ) // <!ELEMENT ATTLIST ENTITY NOTATION
                        FINDNODETYPE( MCD_T(">"), CXmlParser::MNT_DOCUMENT_TYPE )
                    else
            ...
            else if ( nParseFlags & PD_DASH )
            {
                nParseFlags ^= PD_DASH;
                if ( cD == '-' )
                    FINDNODETYPE( MCD_T("-->"), CXmlParser::MNT_COMMENT )
                else
            ...
            else if ( nParseFlags & PD_BRACKET )
            {
                nParseFlags ^= PD_BRACKET;
                if ( cD == 'C' )
                    FINDNODETYPE( MCD_T("]]>"), CXmlParser::MNT_CDATA_SECTION )
                else
            ...
            else if ( nParseFlags & PD_DOCTYPE )
            {
                ...
                {
                    m_nNext = (int)(pD - m_pDocText) + 1;
                    nNodeType = CXmlParser::MNT_DOCUMENT_TYPE;
                    break;
                }
            }
        }
...
CXmlParser::CXmlParser( MCD_CSTR szDoc )
{ 
    x_InitMarkup();
    SetDoc( szDoc );
}

CXmlParser::CXmlParser( int nFlags )
{
    x_InitMarkup(); 
    SetDoc( NULL );
    m_nDocFlags = nFlags; 
...
CXmlParser::CXmlParser( const CXmlParser& markup )
{
    x_InitMarkup();
    *this = markup; 
}
...
void CXmlParser::operator=( const CXmlParser& markup )
{
...
bool CXmlParser::SetDoc( MCD_PCSZ pDoc )
{
...
bool CXmlParser::SetDoc( const MCD_STR& strDoc )
{
...
MCD_STR CXmlParser::GetError() const
{
    ...
    CXmlParser mResult( m_strResult );
    MCD_STR strError;
    int nSyntaxErrors = 0;
...
bool CXmlParser::Load( MCD_CSTR_FILENAME szFileName )
{
    if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
        return false;
...
bool CXmlParser::ReadTextFile( MCD_CSTR_FILENAME szFileName, MCD_STR& strDoc, MCD_STR* pstrResult, int* pnDocFlags, MCD_STR* pstrEncoding )
{
...
bool CXmlParser::Save( MCD_CSTR_FILENAME szFileName )
{
    if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
        return false;
...
bool CXmlParser::WriteTextFile( MCD_CSTR_FILENAME szFileName, const MCD_STR& strDoc, MCD_STR* pstrResult, int* pnDocFlags, MCD_STR* pstrEncoding )
{
...
bool CXmlParser::FindElem( MCD_CSTR szName )
{
    if ( m_nDocFlags & MDF_WRITEFILE )
        return false;
...
bool CXmlParser::FindChildElem( MCD_CSTR szName )
{
    if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
        return false;
...
MCD_STR CXmlParser::EscapeText( MCD_CSTR szText, int nFlags )
{
...
MCD_STR CXmlParser::UnescapeText( MCD_CSTR szText, int nTextLength /*=-1*/ )
{
...
bool CXmlParser::DetectUTF8( const char* pText, int nTextLen, int* pnNonASCII/*=NULL*/, bool* bErrorAtEnd/*=NULL*/ )
{
...
int CXmlParser::DecodeCharUTF8( const char*& pszUTF8, const char* pszUTF8End/*=NULL*/ )
{
...
void CXmlParser::EncodeCharUTF16( int nUChar, unsigned short* pwszUTF16, int& nUTF16Len )
{
...
int CXmlParser::DecodeCharUTF16( const unsigned short*& pwszUTF16, const unsigned short* pszUTF16End/*=NULL*/ )
{
...
void CXmlParser::EncodeCharUTF8( int nUChar, char* pszUTF8, int& nUTF8Len )
{
...
int CXmlParser::UTF16To8( char* pszUTF8, const unsigned short* pwszUTF16, int nUTF8Count )
{
...
int CXmlParser::UTF8To16( unsigned short* pwszUTF16, const char* pszUTF8, int nUTF8Count )
{
...
#if ! defined(MARKUP_WCHAR) // not WCHAR
MCD_STR CXmlParser::UTF8ToA( MCD_CSTR pszUTF8, int* pnFailed/*=NULL*/ )
{
...
MCD_STR CXmlParser::AToUTF8( MCD_CSTR pszANSI )
{
...
MCD_STR CXmlParser::GetDeclaredEncoding( MCD_CSTR szDoc )
{
...
int CXmlParser::GetEncodingCodePage( MCD_CSTR pszEncoding )
{
    return x_GetEncodingCodePage( pszEncoding );
}

int CXmlParser::FindNode( int nType )
{
...
MCD_STR CXmlParser::GetTagName() const
{
...
MCD_STR CXmlParser::GetAttribName( int n ) const
{
...
bool CXmlParser::SavePos( MCD_CSTR szPosName /*=""*/, int nMap /*=0*/ )
{
    if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
        return false;
...
bool CXmlParser::RestorePos( MCD_CSTR szPosName /*=""*/, int nMap /*=0*/ )
{
    if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
        return false;
...
bool CXmlParser::SetMapSize( int nSize, int nMap /*=0*/ )
{
    if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
        return false;
...
bool CXmlParser::RemoveChildElem()
{
    if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
        return false;
...
//////////////////////////////////////////////////////////////////////
// CXmlParser private methods
//
void CXmlParser::x_InitMarkup()
{
    // Only called from CXmlParser constructors
    m_pFilePos = NULL;
...
int CXmlParser::x_GetParent( int i )
{
    return ELEM(i).iElemParent;
}

void CXmlParser::x_SetPos( int iPosParent, int iPos, int iPosChild )
{
...
void CXmlParser::x_SetDebugState()
{
...
bool CXmlParser::x_AllocElemPos( int nNewSize /*=0*/ )
{
...
int CXmlParser::x_ParseElem( int iPosParent, TokenPos& token )
{
...
int CXmlParser::x_FindElem( int iPosParent, int iPos, PathPos& path ) const
{
...
MCD_STR CXmlParser::x_GetPath( int iPos ) const
{
    ...
    while ( iPos )
    {
        ...
        {
            strPath =  MCD_STR(MCD_T("/")) + strTagName + MCD_STR(MCD_T("[")) + COsUtilities::ConvertEncoding(std::to_string(nCount)) + MCD_STR(MCD_T("]")) + strPath;
        }
        iPos = iPosParent;
    }
    return strPath;
...
MCD_STR CXmlParser::x_GetTagName( int iPos ) const
{
...
MCD_STR CXmlParser::x_GetAttrib( int iPos, MCD_PCSZ pAttrib ) const
{
...
bool CXmlParser::x_SetAttrib( int iPos, MCD_PCSZ pAttrib, int nValue, int nFlags /*=0*/ )
{
    // Convert integer to string
    ostringstream  aStream;
    aStream << nValue << ends;
    bool aResult=x_SetAttrib( iPos, pAttrib, COsUtilities::ConvertEncoding(aStream.str()).c_str(), nFlags );
    return aResult;
}

bool CXmlParser::x_SetAttrib( int iPos, MCD_PCSZ pAttrib, MCD_PCSZ pValue, int nFlags /*=0*/ )
{
    if ( m_nDocFlags & MDF_READFILE )
        return false;
    int nNodeStart = 0;
...
bool CXmlParser::x_CreateNode( MCD_STR& strNode, int nNodeType, MCD_PCSZ pText )
{
...
MCD_STR CXmlParser::x_EncodeCDATASection( MCD_PCSZ szData )
{
...
bool CXmlParser::x_SetData( int iPos, int nValue )
{
    // Convert integer to string
    ostringstream  aStream;
    aStream << nValue << ends;
    bool aResult=x_SetData( iPos, COsUtilities::ConvertEncoding(aStream.str()).c_str(), 0 );
    return aResult;
}

bool CXmlParser::x_SetData( int iPos, MCD_PCSZ szData, int nFlags )
{
    if ( m_nDocFlags & MDF_READFILE )
        return false;
    MCD_STR strInsert;
...
MCD_STR CXmlParser::x_GetData( int iPos )
{
    if ( iPos == m_iPos && m_nNodeLength )
    {
    ...
    // If the string is just white spaces then return empty string
    if (strData.find_first_not_of(MCD_T(" \t\n\r")) == string::npos)
    {
        MCD_STRCLEAR(strData);
    }
...
MCD_STR CXmlParser::x_GetElemContent( int iPos ) const
{
    if ( ! (m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE)) )
    {
        ElemPos* pElem = &ELEM(iPos);
...
bool CXmlParser::x_SetElemContent( MCD_PCSZ szContent )
{
    MCD_STRCLEAR(m_strResult);
    if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
        return false;
...
void CXmlParser::x_DocChange( int nLeft, int nReplace, const MCD_STR& strInsert )
{
    x_StrInsertReplace( m_strDoc, nLeft, nReplace, strInsert );
}

void CXmlParser::x_Adjust( int iPos, int nShift, bool bAfterPos /*=false*/ )
{
...
int CXmlParser::x_InsertNew( int iPosParent, int& iPosRel, NodePos& node )
{
...
bool CXmlParser::x_AddElem( MCD_PCSZ pName, int nValue, int nFlags )
{
    // Convert integer to string
    ostringstream  aStream;
    aStream << nValue << ends;
    bool aResult=x_AddElem( pName, COsUtilities::ConvertEncoding(aStream.str()).c_str(), nFlags );
    return aResult;
}

bool CXmlParser::x_AddElem( MCD_PCSZ pName, MCD_PCSZ pValue, int nFlags )
{
    if ( m_nDocFlags & MDF_READFILE )
        return false;
    if ( nFlags & MNF_CHILD )
...
MCD_STR CXmlParser::x_GetSubDoc( int iPos )
{
...
bool CXmlParser::x_AddSubDoc( MCD_PCSZ pSubDoc, int nFlags )
{
...
int CXmlParser::x_RemoveElem( int iPos )
{
...
void CXmlParser::x_LinkElem( int iPosParent, int iPosBefore, int iPos )
{
...
int CXmlParser::x_UnlinkElem( int iPos )
{
...
int CXmlParser::x_UnlinkPrevElem( int iPosParent, int iPosBefore, int iPos )
{
...
int CXmlParser::x_ReleasePos( int iPos )
{
...
int CXmlParser::x_ReleaseSubDoc( int iPos )
{
...
void CXmlParser::x_CheckSavedPos()
{
...
void CXmlParser::x_AdjustForNode( int iPosParent, int iPos, int nShift )
{
...
bool CXmlParser::x_AddNode( int nNodeType, MCD_PCSZ pText, int nNodeFlags )
{
    if ( m_nDocFlags & MDF_READFILE )
        return false;
...
void CXmlParser::x_RemoveNode( int iPosParent, int& iPos, int& nNodeType, int& nNodeOffset, int& nNodeLength )
{
    int iPosPrev = iPos;
