...
//=============================================================================
// to use this feature enable tracing in the makefile and then add 
//  #include "trace-marker.h"
// 
//  and  
//  trace_printk("%s message text\n", __func__);
//
//  the message will be printed to trace log
//=============================================================================
...
{
	if (tracefd == -1) {
		tracefd = open("/sys/kernel/tracing/trace_marker", O_WRONLY);
		if (tracefd == -1) {
			return errno;
		}
	}
