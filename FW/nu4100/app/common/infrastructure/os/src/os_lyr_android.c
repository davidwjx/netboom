...
static int OS_LYRP_thread_compilation();
...
static int OS_LYRP_thread_compilation()
{
   LOGG_PRINT(LOG_ERROR_E, NULL, "__linux__host defined\n");
   LOGG_PRINT(LOG_ERROR_E, NULL, "end of thread_compilation\n");
  return 0;
}
...
static int OS_LYRP_init(void)
{
  /*
   ...
   if (sched_setscheduler(0, SCHED_RR, & param) == 0) {
      printf("changing process policy to SHED_RR (priority=99)\n");   
   } else {
      printf("Warning: changing process priority failed\n. Requires root permission!\n");   
   } 
   */
...
static OS_LYRG_threadHandle OS_LYRP_createThread(OS_LYRG_threadParams *paramsP)
{
   ...
   do
   {
     ...
     scParams.sched_priority = 99;
     ret = pthread_setschedparam(thrd, SCHED_RR, &scParams);
     if (ret != 0)
     {
        //printf("warning: pthread_setschedparam failed for thread (id=%d)\n",paramsP->id);
        ret = setpriority(PRIO_PROCESS, gettid(), -8); //-8 is defined as ANDROID_PRIORITY_URGENT_AUDIO.
        if (ret)
        {
           printf("warning: set priority failed\n");
           break;
        }
        //printf("set thread (id=%d) priority to -8\n", paramsP->id);
     }
...
static int OS_LYRP_closeThread(OS_LYRG_threadHandle thrd)
{
   /* Note: implement alternative to pthread_cancel */
...
static int OS_LYRP_sendMsg(OS_LYRG_msgQueT *msgQueP, UINT8 *msgP, UINT32 len)
{
   ...
   //TODO: This will block if quque is full - use mq_timedSend to avoid blocking
   ret = mq_send(msgQueP->mqHandle,(char *)msgP,len, PRIORITY);
   if(ret < 0)
   {
...
{
   FILE *tmpFile;
   tmpFile = fopen("/storage/emulated/0/Inuitive/my_tmpfile", "w+b");
   if (!tmpFile)
   {
      LOGG_PRINT(LOG_ERROR_E, NULL, "failed to open tempfile at /storage/emulated/0/Inuitive/my_tmpfile\n");
