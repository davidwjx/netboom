...
typedef struct HalContext_s
{
    ...
    uint32_t*           reg_virtual_base;
    unsigned long       reg_phy_base;
    unsigned long       reg_phy_size;
...
RESULT HalGetSensorPixelMode(HalHandle_t HalHandle, uint32_t *pMode);
 ...
 * HalCpuReadReg()
 *****************************************************************************/
INLINE uint32_t HalCpuReadReg( HalHandle_t HalHandle, uint32_t reg_address )
{
    uint32_t read;
    (void) HalHandle;
...
//    printf("HAL_READ:isp %d %08x %08x\n", HalHandleCtx->isp_index, reg_address, read);
 ...
 * HalCpuWriteReg()
 *****************************************************************************/
INLINE void HalCpuWriteReg( HalHandle_t HalHandle, uint32_t reg_address, uint32_t value )
{
    (void) HalHandle;
    ...
    // printf("HAL_WRITE:isp %d %08x %08x\n", HalHandleCtx->isp_index, reg_address, value);
}
...
INLINE uint32_t HalReadReg( HalHandle_t HalHandle, uint32_t reg_address )
{
    ...
    struct HalContext_s * HalHandleCtx = (struct HalContext_s *)HalHandle;
    DCT_ASSERT(HalHandleCtx != NULL);
    feHandle = HalFEGetHandle(HalHandleCtx->isp_index / HAL_CH_NUM);
    ...
    {
        vdId = HalHandleCtx->isp_index & (HAL_CH_NUM-1);
        HalFEReadRegister(feHandle, vdId, reg_address, &read);
    }
...
INLINE void HalWriteReg( HalHandle_t HalHandle, uint32_t reg_address, uint32_t value )
{
    ...
    {
        vdId = HalHandleCtx->isp_index & (HAL_CH_NUM-1);
        HalFEWriteRegister(feHandle, vdId, reg_address, value);
    }
 ...
 * HalWriteTbl()
 *****************************************************************************/
INLINE void HalWriteTbl( HalHandle_t HalHandle, uint32_t reg_address, uint32_t value )
{
    HalFEHandle_t feHandle;
    uint32_t vdId;
    (void) HalHandle;
    ...
    {
        vdId = HalHandleCtx->isp_index & (HAL_CH_NUM-1);
        HalFEWriteTable(feHandle, vdId, reg_address, value);
    }
}
