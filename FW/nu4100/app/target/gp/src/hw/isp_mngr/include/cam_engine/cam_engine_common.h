...
typedef enum CamEngineCmdId_e
{
	...
	CAM_ENGINE_CMD_HW_DMA_CONFIGDONE		= (CAM_ENGINE_CMD_INTERNAL_BASE + 24),	/**< config the image address etc. to dma before load picture */
    CAM_ENGINE_CMD_MAX
} CamEngineCmdId_t;
...
typedef enum CamEnginePathType_e
{
    ...
    CAM_ENGINE_PATH_MCM_WR2   ,
    CAM_ENGINE_PATH_MCM_WR3   ,
    ...
    CAM_ENGINE_PATH_HDR1_L,
    CAM_ENGINE_PATH_HDR1_S,
    CAM_ENGINE_PATH_HDR1_VS,
#endif
#endif
    CAM_ENGINE_PATH_MAX
} CamEnginePathType_t;
...
typedef enum CamEngineConfigType_e
{
    ...
    CAM_ENGINE_CONFIG_USER    = 4,
    CAM_ENGINE_CONFIG_MAX     = 5
...
typedef struct CamEnginePathConfig_s
{
    ...
    CamerIcMiDataYuvBitWidth_t yuv_bit;
...
typedef struct CamEngineWdr5Config_s {
    bool isFlat;
    bool isGlobalCureve;
    
    uint8_t strength;
    uint8_t highStrength;
    uint16_t lowStrength;
    int16_t contrast;
    uint8_t flatLevel;
    uint16_t  entropySlope;
    uint16_t  entropyBase;
    uint8_t flatLevelGlobal;
    uint16_t  fixedWeight;
    uint16_t  maxGain;
    uint16_t  minGain;
    float degamma;
    float diffHigh;
    ...
    float satRange;
    float satThrGainUp;
    float satThrGainDown;
} CamEngineWdr5Config_t;

typedef struct CamEngineAwdr5ParamNode_s
{
    float gain;
    float integrationTime;
    CamEngineWdr5Config_t config;
} CamEngineAwdr5ParamNode_t;
...
#define CAM_ENGINE_VSM_H_SIZE_MIN 64
#define CAM_ENGINE_VSM_V_SIZE_MIN 64
#define CAM_ENGINE_VSM_H_SIZE_MAX 1920
#define CAM_ENGINE_VSM_V_SIZE_MAX 1088
