...
#if FPGA_USE_MMAP
    extern volatile uint32_t* fpga_regs_mmap[HAL_CH_NUM];  /**< memory-mapped registers two isp */
    extern int32_t ispFdAry[HAL_CH_NUM];                       /**< store isp relative fd*/
#endif
...
/** AlteraFPGABoard_WriteTblBAR()
  * @brief  Write a 32-bit DWORD to one of the PCI BARs on the FPGA.
  * @param  bar             the number of the PCI BAR to access
  * @param  address         the address (128-bit aligned) of Table parameters to write to
  * @param  data            the data word to write
  * @retval FPGA_RES_OK     operation successful
  * @retval FPGA_RES_NODEV  FPGA board not initialized (need to call AlteraFPGABoard_Open() first)
  * @retval FPGA_RES_FAIL   could not write value */
uint32_t AlteraFPGABoard_WriteTblBAR(int32_t isp_fd, uint32_t address, uint32_t data);
...
INLINE uint32_t AlteraFPGABoard_ReadReg(int32_t isp_fd, uint32_t address) {
    ...
    for(i = 0; i < HAL_CH_NUM; i++) {
      if (ispFdAry[i] == isp_fd)
        break;
    }
    if (i < HAL_CH_NUM) {
      return fpga_regs_mmap[i][FPGA_REG_ADDRESS_MOD(address) >> 2];
    } else {
...
INLINE uint32_t AlteraFPGABoard_WriteReg(int32_t isp_fd, uint32_t address, uint32_t data) {
    ...
    if (i < HAL_CH_NUM) {
      fpga_regs_mmap[i][FPGA_REG_ADDRESS_MOD(address) >> 2] = data;
    } else {
      return FPGA_RES_FAIL;
    }
    return FPGA_RES_OK;
