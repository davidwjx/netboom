...
private:
    ...
    typedef struct AutoTable {
        vector<float> gain;
        vector<float> backlight;
        vector<float> strength;
        vector<float> highStrength;
        vector<float> lowStrength;
        vector<float> globalStrength;
        vector<float> contrast;
        vector<float> flatStrength;
        vector<float> flatThreshold;
    } AutoTable;

    map<int, AutoTable> autoTable;
    bool useBackLight = false;
};
