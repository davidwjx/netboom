...
#ifdef USB_FOR_NU3000
void *(*nu3000_alloc_trb_cb)(dwc_usb3_pcd_ep_t *pcd_ep, int size);
int (*nu3000_num_trbs)(int ep_num, int ep_dir, int ep_type);
#endif
...
int dwc_usb3_gadget_suspend(dwc_usb3_pcd_t *pcd)
{
   ...
   speed = dwc_usb3_get_device_speed(&g_usb3_dev.pcd);
   system_power_mode((speed == USB_SPEED_SUPER) ? SYSTEM_POWER_SUSPEND_USB3 : SYSTEM_POWER_SUSPEND_USB2);
...
int dwc_usb3_gadget_setup(dwc_usb3_pcd_t *pcd, usb_device_request_t *ctrl)
{
	...
	if (ret < 0 /*&& ret != -DWC_E_NOT_SUPPORTED*/) {
		...
		dwc_usb3_pcd_ep_set_stall(pcd, pcd->ep0);
		pcd->ep0->dwc_ep.stopped = 1;
...
#else
#ifdef USB_FOR_NU3000
	if (nu3000_alloc_trb_cb && (mem = nu3000_alloc_trb_cb(pcd_ep, size)))
		;
	else
#endif
...
#ifdef USB_FOR_NU3000
	if (nu3000_num_trbs)
		num_trbs = nu3000_num_trbs(ep_num, ep_dir, ep_type);
#endif
...
usb_request_t *dwc_usb3_alloc_request(struct dwc_usb3_device *usb3_dev, usb_ep_t *usb_ep)
{
	...
	for (req_idx = 0; req_idx < 4; req_idx++)
#else
...
#ifdef USB_FOR_NU3000
	/* call here any pre-initializations */
	create_descriptors_customization();
#endif
