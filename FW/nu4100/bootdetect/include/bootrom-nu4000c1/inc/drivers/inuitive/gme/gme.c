...
void GMEG_initUsbPhyParam(void)
{
   ...
   // Adjusts the voltage level for the threshold used to detect a disconnect event at the host. To enable tuning at the board level, connect this bus to a register
   //GME_PHY_PARAM_0_COMPDISTUNE0_W(3); --Closed after discussion with Amit
   // This bus adjusts the voltage level for the VBUS<#> valid threshold. To enable tuning at the board level, connect this bus to a register
   //GME_PHY_PARAM_0_OTGTUNE0_W(3);		--Closed after discussion with Amit
   // This static value sets the TX driver de-emphasis value when pipeP_tx_deemph[1:0] is set to 2’b01 (according to the PIPE3 specification).
   // To enable tuning at the board level for RX eye compliance, connect this signal to a register.
   GME_PHY_PARAM_0_PCS_TX_DEEMPH_3P5DB_W(22);
   ...
   GME_PHY_PARAM_0_PCS_TX_DEEMPH_6DB_W(23);
   ...
   GME_PHY_PARAM_1_PCS_TX_SWING_FULL_W(120);
   // Sets the boosted transmit launch amplitude (mVppd). To enable tuning at the board level, connect this bus to a register
   GME_PHY_PARAM_2_TX_VBOOST_LVL_W(4);
   // Sets the number of reference clock cycles to mask the incoming LFPS in U3 and U2 states.
   // Masks the incoming LFPS for the number of reference clock cycles equal to the value of pcs_rx_los_mask_val[9:0].
   // This control filters out short, non-compliant LFPS glitches sent by a non-compliant host.
   // For normal operation, set to a targeted mask interval of 10 µs (value = 10 µs / Tref_clk).
   // If the ref_clkdiv2 signal is used, the value = 10 µs / (2 * Tref_clk). Setting this bus to 10'b0 disables masking. This bus should be accessible to general configuration registers for system testing and debug. The value should be defined when in reset. Changing this value during operation might disrupt normal operation of the link.
   // Value = 10000ns / 41.66ns = 240
   GME_PHY_PARAM_2_PCS_RX_LOS_MASK_VAL_W(240);
