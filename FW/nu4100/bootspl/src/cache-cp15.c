...
void bank_mmu_setup(unsigned int address)
{
   int   i;
   i = address >> 20;

#if defined(CONFIG_SYS_ARM_CACHE_WRITETHROUGH)
      set_sectio n_dcache(i, DCACHE_WRITETHROUGH);
#else
...
{
   int i;
   unsigned int reg,ind;

   /* Set up an identity-mapping for all 4GB, rw for everyone */
   for (i = 0; i < 4096; i++)
      set_section_dcache(i, DCACHE_OFF);

   bank_mmu_setup(DDR_DTB_START_ADDRESS);
   for (ind=0;ind<MAX_KERNEL_SIZE;ind+=0x100000)
      bank_mmu_setup(DDR_KERNEL_START_ADDRESS + ind);
   for (ind=0;ind<MAX_INITRAMFS_SIZE;ind+=0x100000)
      bank_mmu_setup(DDR_INITRAMFS_START_ADDRESS + ind);
    //Enable ROM section
   bank_mmu_setup(0x0);
   //Enable LRAM section
