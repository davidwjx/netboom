...
#define CFG_CONT_RD_MASK	0X01
#define CFG_CONT_RD_ENABLE	0x01
...
#ifdef DEBUG
#define spi_nand_debug(fmt,args...)	debug_printf(fmt ,##args)
#else
#define spi_nand_debug(fmt,args...)
#endif
#define spi_nand_error(fmt,args...)	debug_printf(fmt ,##args)
#define spi_nand_info(fmt,args...)	debug_printf(fmt ,##args)
...
int spi_nand_read_oob(struct spi_nand_chip *chip, loff_t from,
			struct mtd_oob_ops *ops);
int spi_nand_erase(struct spi_nand_chip *chip, u32 addr, u32 len);
