...
unsigned int ddrm_phy_init_seq(unsigned int freq) {
  ...
  GME_SPARE_0_SPARE_BITS_W(0xa03a0001); // write
  ...
  // Spec:
  // "There is a protocol for starting the firmware, and interacting with it to determine the training status.
  //  For more information, refer to the dwc_ddrphy_phyinit_G_execFW() program in the provided code in the phyinit directory".
  
  // Comments inside
  // /tools/snps/dw/dwc_lpddr4_multiphy_v2_tsmc12ffc18_2.00a/Latest/phyinit/Latest/software/lpddr4/src/dwc_ddrphy_phyinit_G_execFW.c
  // Say that training firmware is executed with the following procedure:
  ...
  DDRP_APBONLY_MICROCONTMUXSEL_VAL = 0x1; // rf.DWC_DDRPHYA_APBONLY0.MicroContMuxSel.write(.status(status), .value(1));
  
  // Reset the firmware microcontroller by writing the MicroReset register to
  // set the StallToMicro and ResetToMicro fields to 1 (all other fields should be zero).
  // Then rewrite the registers so that only the StallToMicro remains set (all other fields should be zero).
  DDRP_APBONLY_MICRORESET_VAL = 0x9; // rf.DWC_DDRPHYA_APBONLY0.MicroReset.write(.status(status), .value(9));
  DDRP_APBONLY_MICRORESET_VAL = 0x1; // rf.DWC_DDRPHYA_APBONLY0.MicroReset.write(.status(status), .value(1));
  
  // Begin execution of the training firmware by setting the MicroReset register to 4'b0000.
  DDRP_APBONLY_MICRORESET_VAL = 0x0; // rf.DWC_DDRPHYA_APBONLY0.MicroReset.write(.status(status), .value(0));
  
  // Wait for the training firmware to complete by following the procedure in "uCtrl Initialization and Mailbox Messaging"
  // implemented in dwc_ddrphy_phyinit_userCustom_G_waitFwDone() function.
  res = phyinit_G_waitFwDone(0);
  ...
  if(res == 1)
  {
  	debug_printf("ddrm_phy_init_seq: training FAILED\n");
	return 1;
  }
  ...
  //-------------------------------------------
  // Step D2: Load the 2D IMEM image
  ...
  //-------------------------------------------
  // Step F2: Write the Message Block parameters for the training firmware
  
  // In order for the firmware to run, it must be given some information about the system,
  // the memory configuration, and the memory training steps that it is to run.
  phyinit_F2_loadDMEM(freq);
  ...
  //-------------------------------------------
  GME_SPARE_0_SPARE_BITS_W(0xa03a0002); // write
  return 0;
