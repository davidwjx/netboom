...
static signed int log_2_n_round_up(unsigned int n)
{
	signed int log2n = -1;
	unsigned int temp = n;
...
void invalidate_icache_all(void)
{
	// Perform ISB for safety, even if we are running without I-CACHE
	ARMV7ISB;
}
#endif
...
void __v7_outer_cache_flush_range(unsigned int start, unsigned int end)
{
}
void v7_outer_cache_flush_range(unsigned int start, unsigned int end)
	__attribute__((weak, alias("__v7_outer_cache_flush_range")));

void __v7_outer_cache_inval_range(unsigned int start, unsigned int end)
{
}
