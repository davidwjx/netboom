...
IF NOT DEFINED OPENCV_DIR (
    ...
    if ERRORLEVEL 1 (
        set path "%tmp_path%;%~2\build\x64\vc10\bin" /m >> InstallLog.txt
    	reg.exe ADD "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\Environment" /v Path /t REG_EXPAND_SZ /d "%tmp_path%;%~2\build\x64\vc10\bin" /f
    ) else (
    ...
    where /q opencv_core245.dll    
    if ERRORLEVEL 1 (
        set path "%tmp_path%;%OPENCV_DIR%\build\x64\vc10\bin" /m >> InstallLog.txt
        reg.exe ADD "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\Environment" /v Path /t REG_EXPAND_SZ /d "%tmp_path%;%OPENCV_DIR%\build\x64\vc10\bin" /f
    ) else (
        echo "OPENCV_DIR is already in path" >> InstallLog.txt
    )
)
