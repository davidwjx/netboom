...
static void STEREO_DEMO_finish_operate(inu_fdkH meH, inu_function__operateParamsT *paramsP)
{
    ...
    int i = 0;
    #ifdef DEBUG_TIME_DELTA
    OS_LYRG_getTimeNsec(&endTime);
    INT64 runTime = endTime - startTime;
...
static void STEREO_DEMO_operate(inu_fdkH meH, inu_function__operateParamsT *paramsP)
{
   ...
   if (ERRG_SUCCEEDED(ret))
   {
      ...
      INT64 startTime_TD = currentTime - startTime;
      
      printf("TD: %lld \n", startTime_TD/1000);
      OS_LYRG_getTimeNsec(&startTime);
      #endif
...
int main(int argc, char *argv[])
{
   ...
   /*Cahce invalidation is expensive, it should only be used if data is touched by the ARM core*/
