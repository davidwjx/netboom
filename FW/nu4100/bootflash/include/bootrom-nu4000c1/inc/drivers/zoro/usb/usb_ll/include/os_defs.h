...
typedef struct hs_config_desc_alt_st {
	usb_interface_descriptor_t		iso_intf_desc;
	usb_endpoint_descriptor_t		iso_in_ep_desc;
} UPACKED hs_config_desc_alt_t;

typedef struct ss_config_desc_alt_st {
	usb_interface_descriptor_t		iso_intf_desc;
	usb_endpoint_descriptor_t		iso_in_ep_desc;
	ss_endpoint_companion_descriptor_t	iso_in_ss_ep_comp_desc;
} UPACKED ss_config_desc_alt_t;

typedef struct nu3000_descriptor_override_st{
	u16		wDeviceVendorId;
	u16		wDeviceProductId;
	u8		bConfigMaxPowerHS;
	u8		bConfigMaxPowerSS;
	u8		bConfigPowerFlag;
	u8		bBOSU1ExitLatency;
	u16		wBOSU2ExitLatency;
	u8		containerUniqueId[6];
} UPACKED nu3000_descriptor_override_t;
...
extern const fs_config_desc_t fs_config_desc;
extern const hs_config_desc_t hs_config_desc;
extern const ss_config_desc_t ss_config_desc;
extern int pass_desc_rest(int ss, int (*f)(int, usb_descriptor_header_t *, int, int, void *), void *ctx);
/**
...
#ifndef LINUXTEST
#ifdef USB_FOR_NU3000
extern struct dwc_usb3_device *dwc_usb3_driver_init(u32 base_addr_dwc, u8 irq_num);
#else
