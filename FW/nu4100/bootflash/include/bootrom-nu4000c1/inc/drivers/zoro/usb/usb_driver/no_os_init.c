...
#ifdef LINUXTEST
#define DWC_DRIVER_VERSION	"2.50a - 11/12/2012"
...
/** @{ */
/** PCD request pool */
#if defined(USB_FOR_NU3000) && !defined(BUILD_FOR_MANY_REQUESTS)
dwc_usb3_pcd_req_t			g_pcd_req[4];
#else
...
#else
static char				g_hiber_scratchpad[1][64];
#endif
...
#ifdef USB_FOR_NU3000
	/* check for FPGA mode (maybe by usb_hw_phy2_is_utmi()), maybe change things accordingly in the scope of the gusb2phy* registers */
#endif
...
static void usb3ss_common_irq(void)
{
	dwc_usb3_common_irq(g_usb3_dev.irq_num, &g_usb3_dev);
	irq_clear(g_usb3_dev.irq_num);
}
...
#ifdef USB_FOR_NU3000
void dwc_usb3_common_wakeup(void)
{
	dwc_usb3_device_t *usb3_dev = &g_usb3_dev;
	dwc_usb3_handle_dev_resume(&usb3_dev->pcd);
}
#endif
/**
...
#endif
#ifdef USB_FOR_NU3000
		irq_deregister(usb3_dev->irq_num);
#endif
	}
...
#ifdef LINUXTEST
		dma_free_coherent(&dev->dev, DWC_EVENT_BUF_SIZE, event_buf,
				  event_buf_dma);
#endif
	}
...
#elif defined(USB_FOR_NU3000)
dwc_usb3_device_t *dwc_usb3_driver_init(u32 base_addr_dwc, u8 irq_num)
#else
...
# endif
#ifdef USB_FOR_NU3000
	irq_register(usb3_dev->irq_num, usb3ss_common_irq);
#endif
#endif
...
#ifdef USB_FOR_NU3000
void nu3000_usb_irq_register(int irq_num)
{
   irq_register(irq_num, usb3ss_common_irq);
}
