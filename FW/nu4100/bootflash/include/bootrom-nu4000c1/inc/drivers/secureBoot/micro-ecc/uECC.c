...
#if defined __x86_64__  && uECC_WORD_SIZE == 8 && !defined uECC_NO_X86_64_ASM
#define ASM_LINE(x) x "\n\t"
#define uECC_vli_accumulate4_worker(r0, r1, r2, r3, s0, s1, s2, s3, c_in, c_out) \
    __asm__(ASM_LINE("addq %6, %0") \
            ASM_LINE("adcq %7, %1")  \
            ASM_LINE("adcq %8, %2")  \
            ASM_LINE("adcq %9, %3")  \
            ASM_LINE("adcl $0, %4")  \
...
/* result = result + source mod 2**256; carry_out = carry_in + overflow_flag */
#define UECC_VLI_ACCUMULATE4(result, source, carry_in, carry_out)       \
    uECC_vli_accumulate4_worker((result)[0], (result)[1], (result)[2], (result)[3], \
                         (source)[0], (source)[1], (source)[2], (source)[3], \
                         (carry_in), (carry_out))

#define uECC_vli_accumulate4_neg_worker(r0, r1, r2, r3, s0, s1, s2, s3, c_in, c_out) \
    __asm__(ASM_LINE("subq %6, %0") \
            ASM_LINE("sbbq %7, %1")  \
            ASM_LINE("sbbq %8, %2")  \
            ASM_LINE("sbbq %9, %3")  \
            ASM_LINE("sbbl $0, %4")  \
...
/* result = result + (- source) mod 2**256;
   carry_out = carry_in + underflow_flag */
#define UECC_VLI_ACCUMULATE4_NEG(result, source, carry_in, carry_out) \
    uECC_vli_accumulate4_neg_worker((result)[0], (result)[1], (result)[2], (result)[3], \
                         (source)[0], (source)[1], (source)[2], (source)[3], \
                         (carry_in), (carry_out))
...
#if !asm_add
uECC_VLI_API uECC_word_t uECC_vli_add_implement(uECC_word_t *result,
                                                const uECC_word_t *left,
                                                const uECC_word_t *right,
                                                wordcount_t num_words) {
    uECC_word_t carry = 0;
...
#if !asm_sub
uECC_VLI_API uECC_word_t uECC_vli_sub_implement(uECC_word_t *result,
                                                const uECC_word_t *left,
                                                const uECC_word_t *right,
                                                wordcount_t num_words) {
...
#ifdef USE_CRYPTOPP_INTEGER_FULL
#include "cl_cryptopp_integer_base.h"
#endif /* USE_CRYPTOPP_INTEGER_FULL */

#ifdef USE_CRYPTOPP_INTEGER_MULTIPLY
#include "cl_cryptopp_integer_base.h"
#endif /* USE_CRYPTOPP_INTEGER_MULTIPLY */

#if !asm_mult
#ifdef asm_mult_upto8
uECC_VLI_API void
uECC_vli_mult_upto8(uECC_word_t *result,
                    const uECC_word_t *left,
                    const uECC_word_t *right,
                    wordcount_t num_words);
#endif
...
uECC_VLI_API void uECC_vli_mult(uECC_word_t *result,
    ...
    if (num_words <= 8)
    {
        return uECC_vli_mult_upto8(result, left, right, num_words);
    }
#endif
#ifdef USE_CRYPTOPP_INTEGER_MULTIPLY
    /* Assume CPP integer is the same size than uECC integer. */
    if (num_words == 4 || num_words == 2 || num_words == 8)
    {
        uECC_word_t temp[16] __attribute__((__aligned__(16)));

#ifdef SL_NT_USE_SSE_ALIGN_DATA
        uECC_word_t result_[16] __attribute__((__aligned__(16)));
        uECC_word_t left_[8] __attribute__((__aligned__(16)));
        uECC_word_t right_[8] __attribute__((__aligned__(16)));

        memcpy(left_, left, num_words * sizeof(uECC_word_t));
        memcpy(right_, right, num_words * sizeof(uECC_word_t));
        CL_CryptoPP_Integer_Multiply(
                result_,
                temp,
                left_,
                right_,
                num_words);
        memcpy(result, result_, 2 * num_words * sizeof(uECC_word_t));
#else
        CL_CryptoPP_Integer_Multiply(
                result,
                temp,
                ...
                right,
                num_words);
#endif /* SL_NT_USE_SSE_ALIGN_DATA */
        return;
    }
    /* Special variant of multiplication for P-521. P-521 is
       8 words * 8 words and a few (9) bits more. */
#if uECC_WORD_SIZE == 8
    else if (num_words == 9 && left[8] <= 0x1FF && right[8] <= 0x1FF)
    {
        uECC_word_t temp[16] __attribute__((__aligned__(16)));
        uECC_word_t resulth[8];
        uECC_dword_t tmp = 0;
        wordcount_t i;

        /* 8 * 8 words multiplication. */
        CL_CryptoPP_Integer_Multiply(
                result,
                temp,
                left,
        ...
        for(i = 0; i < 8; i++)
        {
            tmp += ((uECC_dword_t)right[8] * left[i]) +
                ((uECC_dword_t)left[8] * right[i]);
            resulth[i] = (uECC_word_t) tmp;
            tmp >>= uECC_WORD_BITS;
        }

        /* Add carries and right[8] * left[8]. Assume the value is small
           => no mory carry. */
        result[16] = uECC_vli_accumulate(result + 8, resulth, 8) + tmp +
            right[8] * left[8];
        result[17] = 0;
        return;
    }
    ...
    {
        /* Use cl_cryptopp_integer_base.c function dedicated for mult(6). */
        void Baseline_Multiply6(uECC_word_t *R,
                                const uECC_word_t *AA,
                                const uECC_word_t *BB);
        Baseline_Multiply6(result, left, right);
        return;
    }
#endif /* uECC_WORD_SIZE == 8 */
#if uECC_WORD_SIZE == 4 && defined SL_NT_USE_SSE_ALIGN_DATA
    else if (num_words == 6 || num_words == 7)
    {
        uECC_word_t temp[16] __attribute__((__aligned__(16)));
        uECC_word_t result_[16] __attribute__((__aligned__(16)));
        uECC_word_t left_[8] __attribute__((__aligned__(16)));
        ...
        /* Note: Always align the inputs. */
        left_[6] = right_[6] = left_[7] = right_[7] = 0;
        memcpy(left_, left, num_words * sizeof(uECC_word_t));
        memcpy(right_, right, num_words * sizeof(uECC_word_t));

        /* Note: use 8*8 multiplier. */
        CL_CryptoPP_Integer_Multiply(
                result_,
                temp,
                left_,
        ...
        memcpy(result, result_, num_words * 2 * sizeof(uECC_word_t));
        return;
    }
    else if (num_words == 12)
    {
        uECC_word_t temp[32] __attribute__((__aligned__(16)));
        uECC_word_t result_[32] __attribute__((__aligned__(16)));
        uECC_word_t left_[16] __attribute__((__aligned__(16)));
        uECC_word_t right_[16] __attribute__((__aligned__(16)));

        /* Note: Always align the inputs. */
        memcpy(left_, left, 12 * sizeof(uECC_word_t));
        memcpy(right_, right, 12 * sizeof(uECC_word_t));

        left_[12] = right_[12] = result_[24] = 0x33223322;
        void SSE2_Multiply12(uECC_word_t *C, const uECC_word_t *A,
                             const uECC_word_t *B);
        SSE2_Multiply12(result_, left_, right_);

        memcpy(result, result_, 24 * sizeof(uECC_word_t));
#ifdef uECC_OPTIMIZATION_COMPARE
        left_[12] = left_[13] = left_[14] = left_[15] = 0;
        right_[12] = right_[13] = right_[14] = right_[15] = 0;
        uECC_word_t result__[32] __attribute__((__aligned__(16)));
        /* Note: use 16*16 multiplier. */
        CL_CryptoPP_Integer_Multiply(
                result__,
                temp,
                left_,
        ...
        for(i = 0; i < 24; i++)
        {
            if (result__[i] != result_[i])
            {
                printf("Mismatch at idx=%d [%lx vs %lx]\n", i,
                       (unsigned long) result__[i], (unsigned long) result_[i]);
                fail = 1;
            }
        }
        if (result_[24] != 0x33223322)
        {
            printf("magic cookie mismatch @ result[24]\n");
            fail = 1;
        }
    ...
    else if (num_words == 17 && (left[16] | right[16]) <= 0x1FF)
    {
        uECC_word_t temp[32] __attribute__((__aligned__(16)));
        uECC_word_t resulth[16];
        uECC_word_t result_[32] __attribute__((__aligned__(16)));
        uECC_word_t left_[16] __attribute__((__aligned__(16)));
        uECC_word_t right_[16] __attribute__((__aligned__(16)));
        uECC_dword_t tmp = 0;
...
#ifdef uECC_OPTIMIZATION_COMPARE
        /* Compare optimizations against non-optimized path. */
        uECC_word_t left__[32] __attribute__((__aligned__(16)));
        uECC_word_t right__[32] __attribute__((__aligned__(16)));
        uECC_word_t result__[64] __attribute__((__aligned__(16)));

        /* Note: Always align the inputs. */
        memset(left__, 0, 32 * sizeof(uECC_word_t));
        memset(right__, 0, 32 * sizeof(uECC_word_t));
        memcpy(left__, left, 17 * sizeof(uECC_word_t));
        memcpy(right__, right, 17 * sizeof(uECC_word_t));
        /* 8 * 8 words multiplication. */
        CL_CryptoPP_Integer_Multiply(
                result__,
                temp,
        ...
        /* Note: Always align the inputs. */
        memcpy(left_, left, 16 * sizeof(uECC_word_t));
        memcpy(right_, right, 16 * sizeof(uECC_word_t));
        /* 8 * 8 words multiplication. */
        CL_CryptoPP_Integer_Multiply(
                result_,
                temp,
                ...
                right_,
                16);
        memcpy(result, result_, 32 * sizeof(uECC_word_t));

        /* compute 2 * 9bits * 16 */
        for(i = 0; i < 16; i++)
        {
            tmp += ((uECC_dword_t)right[16] * left[i]) +
                ((uECC_dword_t)left[16] * right[i]);
            resulth[i] = (uECC_word_t) tmp;
            tmp >>= uECC_WORD_BITS;
        }

        /* Add carries and right[16] * left[16]. Assume the value is small
           => no more carry to result[33]. */
        result[32] = uECC_vli_accumulate(result + 16, resulth, 16) +
            tmp + right[16] * left[16];
        result[33] = 0;
#ifdef uECC_OPTIMIZATION_COMPARE
        int fail = 0;
        for(i = 0; i < 34; i++)
        {
            if (result__[i] != result[i])
            {
                printf("Mismatch at idx=%d [%lx vs %lx]\n", i,
                       (unsigned long) result__[i], (unsigned long) result[i]);
                fail = 1;
            }
        }
        if (fail)
...
#ifdef USE_CRYPTOPP_INTEGER_SQUARE
#include "cl_cryptopp_integer_base.h"
#endif /* USE_CRYPTOPP_INTEGER_SQUARE */

#ifdef asm_square_upto8
uECC_VLI_API void
uECC_vli_square_upto8(uECC_word_t *result,
                      const uECC_word_t *left,
                      wordcount_t num_words);
#endif
...
uECC_VLI_API void uECC_vli_square(uECC_word_t *result,
    ...
    if (num_words <= 8)
    {
        return uECC_vli_square_upto8(result, left, num_words);
    }
#endif
...
#if uECC_WORD_SIZE == 8
    else if (num_words == 9 && left[8] <= 0x1FF)
    {
        uECC_word_t temp[16] __attribute__((__aligned__(16)));
        uECC_word_t resulth[8];
        uECC_dword_t tmp = 0;
        uECC_word_t double_left8 = 2 * left[8];
        wordcount_t i;
        ...
        for(i = 0; i < 8; i++)
        {
            tmp += ((uECC_dword_t)double_left8 * left[i]);
            resulth[i] = (uECC_word_t) tmp;
            tmp >>= uECC_WORD_BITS;
        }
    ...
    {
        /* Use cl_cryptopp_integer_base.c function dedicated for squaring(6). */
        void Baseline_Square6(uECC_word_t *R, const uECC_word_t *AA);
        Baseline_Square6(result, left);
        return;
    }
#elif uECC_WORD_SIZE == 4
    else if (num_words == 6 || num_words == 7)
    ...
    else if (num_words == 12)
    {
        ...
        void SSE2_Square12(uECC_word_t *C, const uECC_word_t *A);
        SSE2_Square12(result_, left_);

#else
        left_[12] = left_[13] = left_[14] = left_[15] = 0;
        uECC_word_t temp[32] __attribute__((__aligned__(16)));

        /* Note: use 16*16 squaring. */
        CL_CryptoPP_Integer_Square(
                result_,
                temp,
                left_,
    ...
    else if (num_words == 17 && left[16] <= 0x1FF)
    {
        uECC_word_t temp[32] __attribute__((__aligned__(16)));
        uECC_word_t resulth[16];
        uECC_word_t result_[32] __attribute__((__aligned__(16)));
        uECC_word_t left_[16] __attribute__((__aligned__(16)));
        uECC_word_t double_left16 = 2 * left[16];
        uECC_dword_t tmp = 0;
        wordcount_t i;
        ...
        for(i = 0; i < 16; i++)
        {
            tmp += ((uECC_dword_t)double_left16 * left[i]);
            resulth[i] = (uECC_word_t) tmp;
            tmp >>= uECC_WORD_BITS;
        }
...
uECC_VLI_API void uECC_vli_modAdd(uECC_word_t *result,
    ...
    if (carry || uECC_vli_cmp_unsafe(mod, result, num_words) != 1) {
        ...
        uECC_vli_accumulate_neg(result, mod, num_words);
    }
}
...
uECC_VLI_API void uECC_vli_modSub(uECC_word_t *result,
                                  ...
                                  const uECC_word_t *mod,
                                  wordcount_t num_words) {
    uECC_word_t l_borrow;

#ifdef UECC_VLI_ACCUMULATE4_NEG
    /* Fast path for NIST P-256 on x86-64 platform. */
    if (num_words == 4 && result != right)
    {
        int borrow;

        if (left != result)
        {
            memcpy(result, left, 4 * sizeof(uECC_word_t));
        }

        UECC_VLI_ACCUMULATE4_NEG(result, right, 0, borrow);
        if (borrow)
        {
            UECC_VLI_ACCUMULATE4(result, mod, borrow, borrow);
        }
        return;
    }
#elif uECC_WORD_SIZE == 4
    /* Fast path for NIST P-256 on 32-bit platforms. */
    if (result != right)
    {
        /* Optimized path for 192-256 bit primes. */
        ...
        if (num_words == 8)
        {
            ...
            if (left != result)
            {
                memcpy(result, left, 8 * sizeof(uECC_word_t));
            }

            borrow = uECC_vli_accumulate_neg256bit(result, right);
            if (borrow) {
                uECC_vli_accumulate256bit(result, mod);
            }
            return;
        }
        else
        ...
        if (num_words == 7)
        {
            ...
            if (left != result)
            {
                memcpy(result, left, 7 * sizeof(uECC_word_t));
            }

            borrow = uECC_vli_accumulate_neg224bit(result, right);
            if (borrow) {
                uECC_vli_accumulate224bit(result, mod);
            }
            return;
        }
        else
        ...
        if (num_words == 6)
        {
            ...
            if (left != result)
            {
                memcpy(result, left, 6 * sizeof(uECC_word_t));
            }

            borrow = uECC_vli_accumulate_neg192bit(result, right);
            if (borrow) {
                uECC_vli_accumulate192bit(result, mod);
            }
            return;
        }
        else
    ...
    l_borrow = uECC_vli_sub(result, left, right, num_words);
    if (l_borrow) {
        /* In this case, result == -diff == (max int) - diff. Since -x % d == d - x,
           we can get the correct result from result + mod (with overflow). */
        uECC_vli_accumulate(result, mod, num_words);
    }
}
...
uECC_VLI_API void uECC_vli_mmod(uECC_word_t *result,
                                ...
                                const uECC_word_t *mod,
                                wordcount_t num_words) {
    uECC_word_t mod_multiple[2 * uECC_MAX_WORDS] = {0}; /* initialized to suppress clang warning */
...
static void vli_modInv_update(uECC_word_t *uv,
    ...
    uECC_word_t carry = 0;
    if (!EVEN(uv)) {
        carry = uECC_vli_accumulate(uv, mod, num_words);
    }
...
uECC_VLI_API void uECC_vli_modInv(uECC_word_t *result,
    ...
    while ((cmpResult = uECC_vli_cmp_unsafe(a, b, num_words)) != 0) {
        if (EVEN(a)) {
            ...
            uECC_vli_accumulate_neg(a, b, num_words);
            uECC_vli_rshift1(a, num_words);
            if (uECC_vli_cmp_unsafe(u, v, num_words) < 0) {
                uECC_vli_accumulate(u, mod, num_words);
            }
            uECC_vli_accumulate_neg(u, v, num_words);
            vli_modInv_update(u, mod, num_words);
        } else {
            uECC_vli_accumulate_neg(b, a, num_words);
            uECC_vli_rshift1(b, num_words);
            if (uECC_vli_cmp_unsafe(v, u, num_words) < 0) {
                uECC_vli_accumulate(v, mod, num_words);
            }
            uECC_vli_accumulate_neg(v, u, num_words);
            vli_modInv_update(v, mod, num_words);
        }
    }
...
#else
            if (curve->num_bytes == 66) {
                memmove(&public_key[66], &public_key[68], 66);
            }
#endif
            return 1;
        }
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    memcpy((uint8_t *) _private, private_key, num_bytes);
    memcpy((uint8_t *) _public, public_key, num_bytes*2);
#else
    uECC_vli_bytesToNative(_private, private_key, BITS_TO_BYTES(curve->num_n_bits));
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    memcpy((uint8_t *) secret, (uint8_t *) _public, num_bytes);
#else
...
#if uECC_SUPPORT_POINT_VALIDATION
...
static int uECC_sign_with_k(const uint8_t *private_key,
    ...
    uECC_word_t tmp[uECC_MAX_WORDS];
    uECC_word_t tmp_cp[uECC_MAX_WORDS];
    ...
    uECC_make_key((uint8_t *)signature, (uint8_t *)k, curve);
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    memcpy((uint8_t *) tmp, private_key, BITS_TO_BYTES(curve->num_n_bits));
#else
    ...
    memcpy(tmp_cp, message_hash, hash_size);

#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    uECC_vli_nativeToBytes((uint8_t *)tmp, curve->num_bytes, tmp_cp);
#else
    memcpy((uint8_t *)tmp, tmp_cp, curve->num_bytes);
#endif
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    memcpy((uint8_t *) signature + curve->num_bytes, (uint8_t *) s, curve->num_bytes);
#else
...
int uECC_verify(const uint8_t *public_key,
    ...
    uECC_word_t z[uECC_MAX_WORDS] = {0}; /* initialized to suppress clang warning */
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    memcpy((uint8_t *) r, signature, curve->num_bytes);
    memcpy((uint8_t *) s, signature + curve->num_bytes, curve->num_bytes);
#else
    uECC_vli_bytesToNative(_public, public_key, curve->num_bytes);
    uECC_vli_bytesToNative(
    ...
    u1[num_n_words - 1] = 0;
    memcpy(u1, message_hash, hash_size);
    ...
    if (uECC_vli_cmp_unsafe(curve->n, rx, num_n_words) != 1) {
        uECC_vli_accumulate_neg(rx, curve->n, num_n_words);
    }
