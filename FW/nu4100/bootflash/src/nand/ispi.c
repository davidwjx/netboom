...
static UINT32 SPI_DRVP_init_(unsigned int apb_spi_freq_hz)
{
   ...
   if (ERRG_SUCCEEDED(retVal))
   {
     ...
     SPI_N(ISPI_CONFIGURATION_RX_PHASE_W(0));                        // RX data is sampled on the edge defined by the protocol
     ...
     SPI_N(ISPI_CONFIGURATION_CLK_DIV_W(0));
     rel_log("spi rate: %dbps\n",apb_spi_freq_hz);
...
*
****************************************************************************/
UINT32 SPI_DRVG_spiTransaction_(BYTE *pTxBuffer, UINT32 txLen, BYTE *pRxBuffer, UINT32 rxLen, UINT32 flags )
{
   UINT32   retVal = SPI_SUCCESS;
   UINT32 xmt_count = 0;
   UINT32 itrNum = 0, i;
   ...
   if( flags & 0x01 )
   {      
	  SPI_N(ISPI_CS_CS_EN_W(1 << 1)); 
      SPI_N(ISPI_ACTIVATE_CS_ACTIVATE_CS_W(1 << 1));		// TODO:	NOR CS - 0, (NAND CS - 3 ---- on debug board), NAND CS - 1
   }
   ...
   if(xmt_count)
  {
	   debug_printf("%s -TX-: tx len %x, rx len %x \t", __func__, txLen, rxLen);
	   for( i = 0; i < (xmt_count); i++ )
	   {
			debug_printf("%x ", *(pTxBuffer - xmt_count + i));
	   }
	   debug_printf("\n");
  }
#endif
   ...
   if (pRxBuffer && (ERRG_SUCCEEDED(retVal)))
   {
      ...
      {
         if (SPI_N(ISPI_STATUS_RX_ENTRIES_R) > 0)
         {
	 ...
	 if(xmt_count)
	 {
	  debug_printf("%s -RX-: tx len %x, rx len %x \t", __func__, txLen, rxLen);
      for( i = 0; i < (xmt_count); i++ )
	  {
	   debug_printf("%x ", *(pRxBuffer - xmt_count + i));
	  }
	  debug_printf("\n");
	 } 
#endif
   ...
   if( flags & 0x02 )
   {
      SPI_N(ISPI_ACTIVATE_CS_ACTIVATE_CS_W(0));	   // TODO:    NOR CS - 0, NAND CS - 3
   }
   return retVal;
}
