 ...
 ***************       G L O B A L       D A T A              ***************
 ****************************************************************************/
extern unsigned short spi_flags;
...
UINT32 SPI_FLASHG_Read(UINT32 address, UINT32 len, BYTE *pRetBuff)
{
   ...
   if (pRetBuff )
   {
      ...
      {
          if (spi_flags&SPI_NOR_DUAL_READ)
...
//
/////////////////////////////////////////////////////////////
UINT32 SPI_FLASHG_write_cmd(UINT8 opcode)
{
	...
	tempBuff[0] = opcode;
	retVal = SPI_DRVG_spiTransaction(tempBuff, 1, NULL, 0);
	return retVal;
}
